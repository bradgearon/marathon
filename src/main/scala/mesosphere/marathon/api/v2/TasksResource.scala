  ...
  def killTasks(
    ...
    groupedTasks.foreach {
      case (appId, tasks) =>
        ...
        def responseForTasks(killRequested: TaskKiller.KillRequested): Response = Response.ok().build()

        import scala.concurrent.ExecutionContext.Implicits.global
        result(taskKiller.kill(appId, findToKill, scale).map(responseForKill(responseForTasks)))
    }
